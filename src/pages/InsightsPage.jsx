...
export default function InsightsPage() {
  ...
  return (
    <div key={`i-${tabKey}`} style={{ display:"flex", flexDirection:"column", gap:14 }}>

      {/* ── Stat mini-cards ── */}
      <div className="insights-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:12 }}>
        {[
          { label:"Savings Rate",       value: totalIncome > 0 ? `${Math.round((balance/totalIncome)*100)}%` : "—", accent: balance >= 0 ? "#10b981" : "#f43f5e", sub: "of income saved" },
          { label:"Avg. Transaction",   value: txns.length > 0 ? fmt(Math.round((totalIncome+totalExpenses)/txns.length)) : "—", accent: "#6366f1", sub: "per entry" },
          { label:"Top Spend Category", value: top?.label || "—", accent: "#f59e0b", sub: top ? fmt(top.value)+" total" : "—" },
        ].map((c, i) => (
          <div key={i} className="card fade-up" style={{ background:s.card, border:`1px solid ${s.border}`, padding:"20px 22px" }}>
            <div style={{ fontSize:12, color:s.muted, marginBottom:8, fontWeight:600 }}>{c.label}</div>
            <div style={{ fontSize:24, fontWeight:800, color:c.accent, fontFamily:"'JetBrains Mono'" }}>{c.value}</div>
            <div style={{ fontSize:12, color:s.muted, marginTop:6 }}>{c.sub}</div>
          </div>
        ))}
      </div>
      ...
      {breakdown.length > 0 && (
        <div className="card fade-up" style={{ background:s.card, border:`1px solid ${s.border}`, padding:"20px 22px", animationDelay:".1s" }}>
          ...
          <div style={{ height: Math.max(breakdown.length*50+20, 140) }}>
      ...
      {/* ── Balance trajectory ── */}
      <div className="card fade-up" style={{ background:s.card, border:`1px solid ${s.border}`, padding:"20px 22px", animationDelay:".16s" }}>
        ...
        <div className="chart-height-line" style={{ height:210 }}>
          <BalanceLine trend={trend}/>
        </div>
      </div>

      {/* ── Key observations ── */}
      <div className="card fade-up" style={{ background:s.card, border:`1px solid ${s.border}`, padding:"22px 24px", animationDelay:".22s" }}>
        ...
        ].filter(Boolean).map((o, i, arr) => (
          <div key={i} style={{ display:"flex", gap:14, padding:"13px 0", borderBottom: i < arr.length-1 ? `1px solid ${s.border}` : "none" }}>
            <div style={{ width:40, height:40, borderRadius:12, background:o.color+"18", display:"flex", alignItems:"center", justifyContent:"center", fontSize:17, flexShrink:0 }}>
              {o.icon}
            </div>
            <div>
              <div style={{ fontSize:13, fontWeight:700, marginBottom:4 }}>{o.title}</div>
              <div style={{ fontSize:13, color:s.muted, lineHeight:1.65 }}>{o.desc}</div>
            </div>
          </div>
        ))}
      </div>
